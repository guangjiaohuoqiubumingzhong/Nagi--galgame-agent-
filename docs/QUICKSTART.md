...
普通用户选择 Releases 附件 **Nagi-0.3.1-windows-x64.zip**。GitHub 自动生成的
Source code ZIP 和 Nagi-0.3.1-source.zip 是源码，不能当成便携程序双击运行。
...
3. 双击唯一的启动入口 **Nagi.exe**，浏览器会打开本机工作台；无需 Python、Git 或 Codex。
   若系统阻止下载文件，先核对来源与哈希；不要全局关闭安全软件或修改系统执行策略。
4. 重复双击会打开同一个服务。退出用 **Stop Nagi.cmd** 或页面底部“使用说明 → 退出 Nagi 服务”。
   先停止正在执行的任务，等待其保存完成；关闭浏览器本身不会停止后端。

**Nagi.exe** 已嵌入 Nagi 图标。便携包同时保留网页头像，并在 `assets` 目录附带 PNG
原图与多尺寸 ICO 文件。旧版的 `Start Nagi.vbs` 和 `Start Nagi.cmd` 不再提供。
