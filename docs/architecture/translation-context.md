...
2. 构建带调用栈的静态控制流图，区分可能前文与所有路径必经前文。
3. 在范围内按完整对白切块，建立 BM25 和本地 E5 向量索引。
4. 用当前文本及最多四条已确认前文形成查询。QLIE 仅召回必经前文，允许跨脚本／场景；排除当前批次和相邻窗口后再做召回。
...
YU-RIS 479、Ren’Py、KiriKiri、TyranoScript 也已接入统一路线分析。它们沿用各自文本批次流程，
当前从必经原文中使用本地 BM25 召回，按单元 references 引用共享证据池；尚未接入以下 E5／重排流程。
新翻译、拆分重试和查缺补漏使用同一策略；旧任务继续原请求。语法与限制见[路线分析](route-analysis.md)。
...
五种已支持引擎使用入口相对的静态控制流分析。资源包只确定来源和覆盖层，不代表剧情路线。
if/case 的各分支都保留，汇合处计算 may 并集和 must 交集；跨脚本调用按返回栈区分上下文。
仅 must 片段可进入翻译证据，may_only 单独在分析报告中展示。图不完整时不给出必经结论。
条件表达式不求解，因此“可能”是静态图上的可能，尚未排除变量关联导致的不可行路径。
本功能不识别玩家实际存档路线，也不自动命名角色路线。

切块和批次仍按局部 scope 隔离；检索通过必经关系可跨场景／脚本。
QLIE 排除当前批次、相邻窗口和重复证据；其他四个引擎按单元过滤，排除当前批次及重复证据。
支持语法、未知跳转处理、命令和测试见[路线分析](route-analysis.md)。
...
- 语料哈希、切块、策略、模型及路线入口／分析身份进入 tctx_v3 索引 ID，证据内容继续进入翻译请求与候选缓存身份。新旧版本不会混用翻译结果。
