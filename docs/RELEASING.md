...
需要正式发布时，维护者可更新 `release/publish-request.json` 中的版本号并将其提交到 main，
或手动运行 `Publish Nagi portable` 工作流。此操作明确请求公开发布：流程验证版本、运行测试、
打包并搬移启动验证后，先上传草稿，下载校验全部附件，再正式公开。已有同版本发行版会阻止
重复发布，不自动删除、覆盖附件或移动旧标签。发布说明使用 `release/notes-版本号.md`。
普通源码修改不会触发正式发布；只有发布请求文件的变更或显式手动运行才会触发。
