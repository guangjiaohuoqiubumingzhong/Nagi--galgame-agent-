...
def test_agent_can_delegate_risky_approval_to_web_handler(tmp_path):
    approvals = []
    agent = build_agent(
        tmp_path,
        [
            '<tool>{"name":"write_file","args":{"path":"hello.txt","content":"ok"}}</tool>',
            "<final>done</final>",
        ],
        approval_policy="ask",
        approval_handler=lambda name, args: approvals.append((name, args)) or True,
    )

    assert agent.ask("write the file") == "done"
    assert approvals == [("write_file", {"path": "hello.txt", "content": "ok"})]
...
def test_agent_retries_after_empty_model_output(tmp_path):
    ...
    notices = [item["content"] for item in agent.session["history"] if item["role"] == "runtime"]
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".nagi").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".nagi" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "nagi" / "agent" / "welcome" / "screen"
    ...
    assert "MINI CODING AGENT" not in welcome
    assert "nagi" in welcome
    assert "local agent for coding, translation & localization" in welcome
...
def test_ollama_client_posts_expected_payload():
    ...
    class FakeResponse:
        ...
        def read(self):
            return json.dumps({"message": {"role": "assistant", "content": "<final>ok</final>"}}).encode("utf-8")
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "nagi/0.1"
...
def test_openai_chat_client_enables_json_mode_and_extracts_content():
    captured = {}
    ...
    client = OpenAIChatCompatibleModelClient(
    ...
    with patch("urllib.request.urlopen", fake_urlopen):
        result = client.complete("return json", 128)
    ...
    assert client.last_completion_metadata["stop_reason"] == "stop"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "nagi.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("nagi.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = nagi_pkg.build_agent(args)
...
def test_build_agent_uses_right_codes_shared_key_for_openai_provider(tmp_path):
    ...
    with patch.dict(os.environ, {"NAGI_RIGHT_CODES_API_KEY": "sk-right-codes"}, clear=True):
        with patch(
            "nagi.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("nagi.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_arg_parser_leaves_provider_unset_for_runtime_resolution(tmp_path):
    args = nagi_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = nagi_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = nagi_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_project_env_provider_when_cli_omitted(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "NAGI_PROVIDER=openai",
                "NAGI_OPENAI_API_BASE=https://www.right.codes/codex/v1",
                "NAGI_OPENAI_API_KEY=sk-project-openai",
                "NAGI_OPENAI_MODEL=gpt-5.4",
                "NAGI_DEEPSEEK_API_KEY=sk-project-deepseek",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "nagi.cli.AnthropicCompatibleModelClient",
...
def test_build_agent_prefers_cli_provider_over_project_env_provider(tmp_path):
    ...
    args = nagi_pkg.build_arg_parser().parse_args(
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "nagi.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("nagi.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("nagi.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            nagi_pkg.build_agent(args)
...
def test_trace_and_report_redact_secret_env_values(tmp_path):
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": secret}, clear=True):
        ...
        # Exercise redaction independently of the host shell and cleared env.
        agent.tools["run_shell"]["run"] = lambda args: f"exit_code: 0\nstdout:\n{secret}"
...
def test_prompt_budget_metadata_records_budget_decisions(tmp_path):
    ...
    messages = agent.model_client.message_requests[0]

    assert metadata["relevant_memory"]["selected_count"] == 3
    rendered_notes = metadata["relevant_memory"]["rendered_notes"]
    assert len(rendered_notes) == metadata["relevant_memory"]["rendered_count"]
    assert 0 <= len(rendered_notes) <= 3
    assert all(any(note in item["content"] for item in messages) for note in rendered_notes)
    assert messages[0]["role"] == "system"
    assert messages[-1] == {"role": "user", "content": "recall"}
    assert metadata["prompt_chars"] == sum(len(item["content"]) for item in messages)
    assert metadata["prompt_over_budget"] is True  # Rules + current request are protected.
    assert metadata["sections"]["relevant_memory"]["rendered_chars"] <= 80
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .nagi/.\n"
    ...
    index_path = tmp_path / ".nagi" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".nagi" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".nagi" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .nagi/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".nagi" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".nagi" / "memory" / "topics" / "dependency-facts.md"
...
def test_all_conversation_turns_are_preserved_when_they_fit(tmp_path):
    agent = build_agent(tmp_path, ["<final>Done.</final>"])
    old_text = "OLD-" + ("A" * 320)
    ...
    assert {"role": "user", "content": old_text} in messages
    assert {"role": "assistant", "content": old_text} in messages
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(nagi_pkg.__file__).as_posix().endswith("/nagi/__init__.py")
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(nagi_pkg.main)
    assert callable(nagi_pkg.build_agent)
    assert callable(nagi_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "nagi", "--help"],
        capture_output=True,
        text=True,
    )
