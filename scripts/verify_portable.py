...
bundle.rename(moved)
for asset in (
    'assets/nagi-icon.ico',
    'assets/nagi-icon.png',
    'runtime/Lib/site-packages/nagi/web_ui/nagi-avatar.png',
):
    assert (moved / asset).is_file() and (moved / asset).stat().st_size > 0, asset
assert (moved / 'Nagi.exe').is_file()
assert not (moved / 'Start Nagi.vbs').exists()
assert not (moved / 'Start Nagi.cmd').exists()
import win32gui
large_icons, small_icons = win32gui.ExtractIconEx(str(moved / 'Nagi.exe'), 0)
assert large_icons or small_icons
