...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = workspace_state(workspace_root) / "memory" if workspace_root is not None else None
...
def retrieval_candidates(state, query, limit=3, workspace_root=None):
    ...
    if workspace_root is not None:
        durable_store = DurableMemoryStore(workspace_state(workspace_root) / "memory")
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(workspace_state(workspace_root) / "memory") if workspace_root is not None else None
